    ...
    <div class="container mx-auto px-4 pt-32 pb-16">
        ...
        <div class="text-center mb-8 sm:mb-12">
            <div class="w-16 h-16 sm:w-20 sm:h-20 mx-auto mb-3 sm:mb-4 bg-gradient-to-br from-amber-400 to-orange-500 rounded-full flex items-center justify-center shadow-lg">
                <i class="fas fa-calendar-alt text-2xl sm:text-3xl text-white"></i>
            </div>
            ...
            <p class="text-base sm:text-xl text-gray-700 max-w-2xl mx-auto px-4">
                Lihat jadwal booking lapangan secara real-time
            </p>
        </div>
        ...
        <div class="mb-6 sm:mb-8 fade-in">
            ...
            <div class="flex flex-wrap justify-center gap-2 sm:gap-3 mb-4 sm:mb-6">
                @foreach($courts as $court)
                    <button 
                        ...
                        class="court-tab px-3 sm:px-6 py-2 sm:py-3 rounded-xl font-semibold transition-all duration-300 inline-flex items-center space-x-1 sm:space-x-2 text-sm sm:text-base
                        ...
                        <i class="{{ $court->sport->icon }} text-base sm:text-lg"></i>
            ...
            <!-- Selected Court Info -->
            <div class="text-center bg-white rounded-xl p-3 sm:p-4 shadow-md max-w-md mx-auto mt-3 sm:mt-4 mb-4 sm:mb-5">
                <h3 class="text-base sm:text-lg font-semibold text-amber-800" id="selected-court-info">
                    <i class="{{ $selectedCourt->sport->icon }} mr-2"></i>
                    {{ $selectedCourt->sport->name }} - {{ $selectedCourt->name }}
                </h3>
                <p class="text-xs sm:text-sm text-gray-600 mt-1" id="selected-date-info">
            ...
            <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6">
                <h3 class="text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4 text-center">
                    <i class="fas fa-calendar mr-2"></i>Pilih Tanggal
                </h3>
                <div class="relative">
                    <div class="flex overflow-x-auto scrollbar-hide space-x-2 sm:space-x-3 py-2 px-2" id="dateCardsContainer">
                        <!-- Date cards will be generated by JavaScript -->
                    </div>
                    <!-- Scroll buttons - Hidden on mobile -->
                    <button id="scrollLeft" class="hidden sm:block absolute left-0 top-1/2 transform -translate-y-1/2 bg-white shadow-lg rounded-full p-2 hover:bg-gray-50 transition-colors">
                        <i class="fas fa-chevron-left text-gray-600"></i>
                    </button>
                    <button id="scrollRight" class="hidden sm:block absolute right-0 top-1/2 transform -translate-y-1/2 bg-white shadow-lg rounded-full p-2 hover:bg-gray-50 transition-colors">
        ...
        <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6 fade-in">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 gap-3">
                <h3 class="text-lg sm:text-xl font-bold text-gray-800">
                    <i class="fas fa-clock mr-2"></i>Slot Waktu
                </h3>
                <div class="flex items-center space-x-3 sm:space-x-4 text-xs sm:text-sm">
                    <span class="inline-flex items-center">
                        <div class="w-2 h-2 sm:w-3 sm:h-3 bg-green-400 rounded-full mr-1 sm:mr-2"></div>
                        <span class="hidden sm:inline">Tersedia</span>
                    </span>
                    <span class="inline-flex items-center">
                        <div class="w-2 h-2 sm:w-3 sm:h-3 bg-red-400 rounded-full mr-1 sm:mr-2"></div>
                        <span class="hidden sm:inline">Dibooking</span>
                    </span>
                    <span class="inline-flex items-center">
                        <div class="w-2 h-2 sm:w-3 sm:h-3 bg-indigo-400 rounded-full mr-1 sm:mr-2"></div>
            ...
            <!-- Loading State -->
            <div id="loading-state" class="text-center py-6 sm:py-8 hidden">
                <div class="loading-spinner mx-auto mb-3"></div>
                <p class="text-sm sm:text-base text-gray-600">Memuat jadwal...</p>
            </div>
            
            <!-- Time Slots Container -->
            <div id="time-slots-container" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2 sm:gap-4">
                @foreach($timeSlots as $slot)
                    <div class="time-slot {{ $slot['is_available'] ? 'available' : 'booked' }} p-3 sm:p-4 rounded-xl text-center"
                         onclick="{{ $slot['is_available'] ? 'showSlotDetails(' . json_encode($slot) . ')' : 'showBookingDetails(' . json_encode($slot) . ')' }}">
                        <div class="font-bold text-base sm:text-lg mb-1">{{ $slot['start_time'] }}</div>
                        <div class="text-xs sm:text-sm opacity-80 mb-1 sm:mb-2">{{ $slot['price_label'] }}</div>
                        <div class="text-xs font-semibold">
                            @if($slot['is_available'])
                                <span class="sm:hidden">{{ number_format($slot['price'] / 1000, 0) }}k</span>
                                <span class="hidden sm:inline">Rp {{ number_format($slot['price'], 0, ',', '.') }}</span>
                            @else
                                <span class="truncate block">{{ $slot['booking_info']['team_name'] ?? 'Dibooking' }}</span>
                            @endif
                        </div>
                        @if(!$slot['is_available'])
                            <div class="text-xs mt-1 opacity-60 hidden sm:block">
            ...
            <!-- No Slots Message -->
            <div id="no-slots-message" class="text-center py-8 sm:py-12 hidden">
                <i class="fas fa-calendar-times text-3xl sm:text-4xl text-gray-400 mb-3 sm:mb-4"></i>
                <p class="text-base sm:text-lg font-semibold text-gray-600">Tidak ada slot waktu tersedia</p>
        ...
        <div class="mt-6 sm:mt-8 text-center">
            <a href="{{ route('booking.index') }}" 
               class="inline-flex items-center bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white font-semibold px-6 sm:px-8 py-2.5 sm:py-3 rounded-xl transition-all duration-300 hover:shadow-lg hover:scale-105 text-sm sm:text-base">
